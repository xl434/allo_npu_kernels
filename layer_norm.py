...
def _test_layer_norm_tiling():
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        print("PASSED Layer Norm!")
    else:
        print("MLIR_AIE_INSTALL_DIR unset. Skipping AIE backend test.")


def _test_layer_norm_single_tile():
    ...
    input_tensor = torch.randn(seq_tile, hidden_size, dtype=torch.float32)
    weight = torch.randn(hidden_size, dtype=torch.float32)
    output = layernorm(input_tensor, weight)

    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        mod = df.build(top, target="aie")
        output_allo = np.zeros((seq_tile, hidden_size)).astype(np.float32)
        mod(input_tensor.cpu().numpy(), weight.cpu().numpy(), output_allo)
        np.testing.assert_allclose(output_allo, output, rtol=1e-2)
        print("PASSED Layer Norm!")
    else:
...
if __name__ == "__main__":
    _test_layer_norm_single_tile()
    _test_layer_norm_tiling()